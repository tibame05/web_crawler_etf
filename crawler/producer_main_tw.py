...
def main_tw() -> Dict[str, Any]:
    ...
    D) STEP3：執行回測
        ...
        - 將回測結果寫入資料庫。
    E) STEP4：收尾
        - 更新所有已處理的 ETF 在 `etl_sync_status` 中的 `updated_at` 時間戳。
    回傳：
    {
    "summary": {...},
    "per_etf": {
    ...
    for eid in tri_etf_ids:
        try:
            ...
            # 3) 決定是否回測：
            #    規則：只有「有新 TRI」（last_tri_date）才回測；否則跳過（等到 E）
            do_backtest = (last_tri_date_new)
            if not do_backtest:
    ...
    try:
        ...
        write_etl_sync_status_to_db(rows_to_update)
