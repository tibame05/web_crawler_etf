...
@app.task(name="crawler.tasks_etf_list_us.fetch_us_etf_list")
def fetch_us_etf_list(crawler_url: str = "https://tw.tradingview.com/markets/etfs/funds-usa/", region: str = "US"):
    ...
    with SessionLocal.begin() as session:
        ...
        etf_records = []
        # 解析表格數據
        rows = soup.find_all("span", {"class": "tickerCell-ixuo49jq"})

        for row in rows:
            code_tag = row.find("a", {"class": "tickerNameBox-ixuo49jq"})
            name_tag = row.find("a", {"class": "tickerDescription-ixuo49jq"})
