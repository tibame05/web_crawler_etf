...
# Celery task 包裝
@app.task(name="crawler.tasks_fetch.fetch_daily_prices_task")
def fetch_daily_prices_task(
    etf_id: str,
    plan: Dict[str, Any],
) -> Optional[Dict[str, Any]]:
    return fetch_daily_prices(etf_id=etf_id, plan=plan)

@app.task(name="crawler.tasks_fetch.fetch_dividends_task")
def fetch_dividends_task(
    etf_id: str,
    plan: Dict[str, Any],
    region: str,
) -> Optional[Dict[str, Any]]:
    return fetch_dividends(etf_id=etf_id, plan=plan, region=region)