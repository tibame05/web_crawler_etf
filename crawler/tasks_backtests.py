...
# Celery task 包裝
@app.task(name="crawler.tasks_backtests.backtest_windows_from_tri_task")
def backtest_windows_from_tri_task(
    etf_id: str,
    end_date: str,
    windows_years: Optional[Iterable[int]] = None,
    risk_free_rate_annual: float = 0.0,
    annualization: int = 252,
) -> Dict[str, object]:
    """
    一般版：直接指定 end_date。
    注意：不傳 session，沿用原函式的 session=None 預設。
    """
    if windows_years is None:
        windows_years = BACKTEST_WINDOWS_YEARS

    if not end_date:
        logger.info("[BACKTEST][%s] 無 end_date，全部跳過。", etf_id)
        return {
            "etf_id": etf_id,
            "end_date": None,
            "written": 0,
            "windows_done": [],
            "windows_skipped": [f"{y}y" for y in windows_years],
            "status": "skipped_no_end_date",
        }

    return backtest_windows_from_tri(
        etf_id=etf_id,
        end_date=end_date,
