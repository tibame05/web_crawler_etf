...
美股整體管線（US only）
核心流程：
1. 名單對齊：同步 tradingview US 與 DB 資訊。
2. 任務派發：規劃並抓取價格與股利。
...
from crawler.rpc_tasks import (
    rpc_fetch_us_etf_list,
    rpc_align_step0_us,
    rpc_plan_price_fetch_us,
    rpc_plan_dividend_fetch_us,
    rpc_async_fetch_daily_prices_us,
    rpc_async_fetch_dividends_us,
    rpc_async_build_tri_us,
    rpc_async_backtest_windows_us
)
...
def main_us() -> Dict[str, Any]:
    ...
    result: Dict[str, Any] = {
        "summary": {
            "region": REGION_US, "start_date": DEFAULT_START_DATE,
    ...
    try:
        ...
        with SessionLocal.begin() as session:
            ...
            # 使用 RPC 封裝
            src_rows = rpc_fetch_us_etf_list(crawler_url="https://tw.tradingview.com/markets/etfs/funds-usa/")
            logger.info("步驟 A.1：自 tradingview 股市成功爬取 %d 筆原始 ETF 名單。", len(src_rows))

            etfs_data_list = rpc_align_step0_us(src_rows)
            ...
            try:
                # 呼叫封裝後的函式
                n_new = init_missing_sync_status(active_ids, REGION_US, session)
        ...
        with SessionLocal.begin() as session:
            ...
            for eid in active_ids:  
                ...
                plans[eid] = {
                    "p": rpc_plan_price_fetch_us(eid, inception_date),
                    "d": rpc_plan_dividend_fetch_us(eid, inception_date)
                }
            ...
            for eid in active_ids:
                if plans[eid]["p"]:
                    ...
                    price_jobs[eid] = rpc_async_fetch_daily_prices_us(eid, plans[eid]["p"])
                
                if plans[eid]["d"]:
                    plan_d = {"start": plans[eid]["d"]["start"]}
                    logger.info("[%s] 步驟 B.2.2：執行股利資料抓取 (計畫=%s)...", eid, plan_d)
                    div_jobs[eid] = rpc_async_fetch_dividends_us(eid, plans[eid]["d"])
            ...
            for eid in active_ids:
                try:
                    ...
                    if new_records_p > 0:
                        tri_jobs[eid] = rpc_async_build_tri_us(eid)
                    else:
            ...
            for eid, job in tri_jobs.items():
                try:
                    ...
                    if tri_added > 0:
                        bt_jobs[eid] = rpc_async_backtest_windows_us(eid, tri_res.get("last_tri_date"), BACKTEST_WINDOWS_YEARS)
                    else:
